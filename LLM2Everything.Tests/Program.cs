...
var tests = new List<(string Name, Func<Task> Body)>
{
    ...
    ("高速ルール: 先週月曜日の10GB以上のGGUF", async () => await ParseAssert("先週月曜日の10GB以上のgguf", r => r.Intent.Size.MinBytes == 10737418240L && r.Intent.Extensions.Contains("gguf") && r.Intent.Modified.Label == "先週月曜日" && r.Intent.Modified.Start!.Value.Day == 8)),
    ...
    ("日時: 先週月曜日", () => DateAssert(d => d.DayOfLastWeek(new DateTimeOffset(2026, 6, 18, 12, 0, 0, TimeSpan.FromHours(9)), DayOfWeek.Monday).Start!.Value.Day == 8)),
